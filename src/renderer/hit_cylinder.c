...
int		quadratic_cylinder(t_quad_cyl *quad, t_objs *cyl, t_ray ray, int t_max)
{
	...
	quad->m = dot_product(ray.dir, cyl->normal) * quad->t + dot_product(quad->x, cyl->normal);
	if ((quad->t <= T_MIN || quad->t >= t_max) || (quad->m < T_MIN || quad->m > cyl->height))
	{
		quad->t = (-quad->half_b + quad->sqrtd) / quad->a;
		quad->m = dot_product(ray.dir, cyl->normal) * quad->t + dot_product(quad->x, cyl->normal);
		if ((quad->t < T_MIN || quad->t >= t_max) || (quad->m < T_MIN || quad->m > cyl->height))
...
t_hitrec	hit_cylinder(t_objs	*cyl, t_ray ray, int t_max)
{
	...
	hitrec.normal = normalize(vec_sub_vec(vec_sub_vec(hitrec.hitpoint, cyl->center), vec_mult(quad.m, cyl->normal)));
