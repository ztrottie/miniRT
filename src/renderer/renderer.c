...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/21 19:58:19 by zbeaumon          #+#    #+#             */
/*   Updated: 2023/11/21 19:58:20 by zbeaumon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_gen(t_data *data, t_rend *rend)
{
	...
	vp = &data->final_vp;
	rend->delta = vec_add(vec_mult(rend->i, vp->delta_v), \
	vec_mult(rend->j, vp->delta_u));
...
void	ray_loop(t_data *data)
{
	...
	while (rend.j < data->img_height)
	{
		...
		{
			ray_gen(data, &rend);
			mlx_put_pixel(data->mlx_image, rend.i, rend.j, \
			put_color(data, rend.ray));
			rend.i++;
		}
		rend.j++;
	}
