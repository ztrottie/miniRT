...
t_hitrec	hit_light(t_data *data, t_hitrec hitrec)
{
	...
	vec_dist(hitrec.hitpoint, data->light.center));
	if (hitrec2.hit)
		hitrec.material.color = ambient_color;
	else
	{
		bright = dot_product(hitrec.normal, ray_direction) \
		* data->light.material.bright;
		hitrec.material.color = vec_add(vec_mult(bright, \
		hitrec.material.color), ambient_color);
	}
	return (hitrec);
}
